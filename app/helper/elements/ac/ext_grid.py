...
class ExtGridHandler:
    """
    External Grid (slack).

    Input contract (node.data):
    - busId: str (required)  # reactflow node id của bus
    - vm_pu: float (optional, default 1.0)
    - va_degree: float (optional, default 0.0)
    - name: str (optional)
    ...
    def create(self, ctx: ElementContext, node: NodeDict) -> Optional[int]:
        ...
        bus_id = str(data.get("busId") or "").strip()
        name = str(data.get("name") or "Ext Grid")
