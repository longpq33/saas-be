...
class GenHandler:
    ...
    - busId: str (required)
    - p_mw: float (required)
    - vm_pu: float (optional, default 1.0)
    - min_q_mvar/max_q_mvar: float (optional)
    - name: str (optional)
    ...
    def validate(self, ctx: ElementContext, node: NodeDict) -> bool:
        ...
        for f in ("p_mw", "vm_pu", "min_q_mvar", "max_q_mvar"):
    ...
    def create(self, ctx: ElementContext, node: NodeDict) -> Optional[int]:
        ...
        min_q = data.get("min_q_mvar")
        max_q = data.get("max_q_mvar")
        min_q_mvar = float(min_q) if min_q is not None else None
        max_q_mvar = float(max_q) if max_q is not None else None
        ...
        try:
            ...
            ctx.add_error(element_id=node_id, element_type=self.element_type, message=f"Lỗi tạo gen: {e}")
