...
class LoadHandler:
    ...
    - controllable: bool (optional, default False)
    - name: str (optional)
    ...
    def create(self, ctx: ElementContext, node: NodeDict) -> Optional[int]:
        ...
        bus_id = str(data.get("busId") or "").strip()
        name = str(data.get("name") or "Load")
        ...
        try:
            ...
            ctx.add_error(element_id=node_id, element_type=self.element_type, message=f"Lỗi tạo load: {e}")
