...
class MotorHandler:
    ...
    - busId: str (required)
    - pn_mech_mw: float (required)
    - cos_phi: float (optional, default 0.85)
    - efficiency: float (optional, default 0.9)
    - loading_percent: float (optional, default 100.0)
    - name: str (optional)
    ...
    def validate(self, ctx: ElementContext, node: NodeDict) -> bool:
        ...
        if data.get("pn_mech_mw") is None:
            ctx.add_error(element_id=node_id, element_type=self.element_type, field="pn_mech_mw", message="Thiếu pn_mech_mw.")
            ctx.set_status_fail(element_id=node_id, element_type=self.element_type, error="missing pn_mech_mw")
            return False

        for f in ("pn_mech_mw", "cos_phi", "efficiency", "loading_percent"):
    ...
    def create(self, ctx: ElementContext, node: NodeDict) -> Optional[int]:
        ...
        bus_id = str(data.get("busId") or "").strip()
        name = str(data.get("name") or "Motor")
        pn_mech_mw = float(data.get("pn_mech_mw"))
        ...
        try:
            ...
            ctx.add_error(element_id=node_id, element_type=self.element_type, message=f"Lỗi tạo motor: {e}")
